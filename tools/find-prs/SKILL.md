...
|------|------|
| `--no-copy` | スキップ（告知不要） |
| Windows 以外 | スキップし、完了報告で「macOS/Linux ではクリップボード未対応のためスキップしました」と告知 |
| `--format` に oneline 未含有 | スキップし、完了報告で「コピー対象（oneline）が出力に含まれないためスキップしました」と告知 |
